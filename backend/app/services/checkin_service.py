    ...
    def create_checkin(
        ...
        *,
        ...
        if checkin_method == "online" and patient.suspended_until and patient.suspended_until >= date.today():
            logger.warning(f"報到失敗: patient_id={patient_id} 被限制線上報到，直到 {patient.suspended_until}。")
            raise HTTPException(
        ...
        if appointment.status not in ["scheduled", "confirmed"]:
            logger.warning(f"報到失敗: 預約 {appointment_id} 狀態為 '{appointment.status}'，無法報到。")
            raise HTTPException(
        ...
        if appointment.date != date.today():
            logger.warning(f"報到失敗: 預約 {appointment_id} 日期 {appointment.date} 不為今天 {date.today()}。")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="只能在預約當天報到。"
            )
        ...
        # Use doctor_id as room_id for simplicity, as per SDD's RoomDay definition
        room_id = appointment.doctor_id
        today = date.today()
        logger.info(f"嘗試獲取或創建 RoomDay 記錄，room_id={room_id}, date={today}。")

        # Get or create RoomDay for today and lock it for update
        db.begin_nested() # Start a nested transaction for RoomDay operations
        try:
            db_room_day = crud_room_day.get_by_room_id_and_date(db, room_id=room_id, date=today)
            
            if not db_room_day:
                logger.info(f"RoomDay 記錄不存在，為 room_id={room_id}, date={today} 創建新的 RoomDay 記錄。")
                db_room_day = crud_room_day.create(db, obj_in=RoomDayCreate(room_id=room_id, date=today, next_sequence=1))
            
            # Atomically increment next_sequence
            # We need to re-fetch with FOR UPDATE to ensure we have the latest locked version
            db_room_day_locked = crud_room_day.get_room_day_for_update(db, room_id=room_id, date=today)
            if not db_room_day_locked: # Should not happen if created above, but for safety
                logger.error(f"報到失敗: 無法獲取 RoomDay 鎖定，room_id={room_id}, date={today}。")
        ...
        crud_appointment.update_status(db, appointment_id=appointment.appointment_id, new_status="checked_in")
