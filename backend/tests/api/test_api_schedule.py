...
def test_create_schedule(client: TestClient, db: Session) -> None:
    ...
    data = {"doctor_id": str(doctor.doctor_id), "date": "2025-11-20", "start": "09:00:00", "end": "12:00:00"}
...
def test_list_schedules(client: TestClient, db: Session) -> None:
    ...
    headers = {"Authorization": f"Bearer {token}"}
    doctor = create_random_doctor(db)
    schedule_in = ScheduleCreate(doctor_id=doctor.doctor_id, date=date(2025, 11, 21), start=time(9,0), end=time(12,0))
    ...
    schedule_in_dict['start'] = schedule_in_dict['start'].isoformat()
    schedule_in_dict['end'] = schedule_in_dict['end'].isoformat()
