...
| 5.5 | 存取控管（RBAC） | 系統支援基於角色的存取控管，細分讀寫權限 |
...
- 主要流程：
  ...
  範例：若病患原本號碼為 3，但在他過號期間系統目前已叫到 5，則重新報到時系統會將該原號（3）插入到目前叫號後第 3 位（即插入於 9 號之前），使得後續順序由 6,7,8,3,9（以此類推）。
- 替代/異常流程：
...
### Use Case 4.3: 過號/爽約懲罰 (No-show/Missed Appointment Penalty)

#### 活動圖 (Activity Diagram)

```mermaid
flowchart TD
    A[開始] --> B{系統識別預約時間已過<br>但未完成看診的預約};
    B -- 有 --> C[將預約狀態標記為 'no_show'];
    B -- 沒有 --> Z[結束];
    C --> D[為病患建立一筆 'no_show' 的違規紀錄];
    D --> E{檢查該病患<br>累積的 'no_show' 紀錄數量};
    E -- "數量 >= 懲罰門檻 (例如 3 次)" --> F[套用懲罰];
    E -- "數量 < 門檻" --> G[發送警告通知給病患];
    F --> H[更新病患資料，設定 'suspended_until' 日期];
    H --> I[發送停權通知給病患<br>(告知限制線上報到)];
...
此活動圖描述了系統自動處理病患「未到診 (no-show)」並執行相應懲罰的流程。

1.  **識別與記錄 (每日或每診結算流程)**：
    *   流程由一個自動化作業觸發，例如在每日結束或每個門診診次結束後。
    *   系統會掃描所有預約時間已過，但狀態仍非「已完成」或「已取消」的預約。
    *   對於符合條件的預約，系統會將其狀態更新為「no_show」(未到診)。
    *   接著，系統會在 `Infraction` (違規紀錄) 資料表中為該病患新增一筆 `no_show` 型態的紀錄。

2.  **懲罰觸發與執行 (懲罰觸發流程)**：
    *   在新增違規紀錄後，系統會立即檢查該病患累積的 `no_show` 紀錄總數。
    *   **如果違規次數小於預設的懲罰門檻** (例如少於 3 次)，系統會向病患發送一則警告通知，提醒其本次的未到診紀錄，流程結束。
    *   **如果違規次數達到或超過懲罰門檻**，系統將啟動懲罰機制。

3.  **執行懲罰 (懲罰執行)**：
    *   系統會更新該病患在 `PATIENT` 資料表中的 `suspended_until` 欄位，設定一個未來的日期。在此日期之前，該病患將被限制部分功能。
    *   最後，系統會向病患發送一則正式的停權通知，明確告知其線上報到功能已被暫時限制，以及限制的截止日期。接著流程結束。
```
} measures the difference between the old_string and the potentially_problematic_new_string. The goal is to identify and correct any syntax errors in the new_string that might arise from incorrect escaping. This ensures that the replacement operation is successful and the new_string is correctly interpreted in the target file. The output should be a JSON object containing the corrected new_string. If no corrections are needed, the original new_string should be returned. The JSON object should have a single key: